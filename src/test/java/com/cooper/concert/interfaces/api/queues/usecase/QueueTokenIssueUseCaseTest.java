...
class QueueTokenIssueUseCaseTest {
	...
	void 유저_대체_식별자를_입력하면_대기열_토큰_발급_성공() {
		...
		when(queueTokenIssueService.issueQueueToken(any())).thenReturn(token);
		when(waitingQueueService.enqueueUserId(any())).thenReturn(position);
		...
		assertSoftly(softAssertions -> {
			softAssertions.assertThat(sut.token()).isNotNull();
	...
	void 대기_토큰이_존재하면_기존_토큰_취소() {
		...
		// then
		assertSoftly(softAssertions -> {
			Mockito.verify(waitingQueueService, Mockito.times(1)).removeFromWaitingQueue(any());
			softAssertions.assertThat(sut.token()).isNotNull();
			softAssertions.assertThat(sut.waitingPosition()).isEqualTo(position);
		});
	}
